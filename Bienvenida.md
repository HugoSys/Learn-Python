
# Learn-Python
```text
  _                           ____        _   _                 
 | |    ___  __ _ _ __ _ __  |  _ \ _   _| |_| |__   ___  _ __  
 | |   / _ \/ _` | '__| '_ \ | |_) | | | | __| '_ \ / _ \| '_ \ 
 | |__|  __/ (_| | |  | | | ||  __/| |_| | |_| | | | (_) | | | |
 |_____\___|\__,_|_|  |_| |_||_|    \__, |\__|_| |_|\___/|_| |_|
                                    |___/                                     
```

# 🐍 Mi Ruta de Aprendizaje: De Cero a Experto

Bienvenido a mi repositorio de aprendizaje de Python. Mi nombre es Hugo Muro y este espacio documenta mi progreso, contiene mis apuntes, ejercicios prácticos y proyectos mientras domino este lenguaje de programación, cabe recalcar que este aprendizaje es en base a mis dos cursos de Cisco, llamados Python 1 y Python 2.

## Propósito de este repositorio

* Documentar mi proceso de aprendizaje.
* Mantener mi código organizado y accesible.
* Construir un portafolio de proyectos prácticos.


## ¿Cómo usar este repositorio localmente?

Si quieres clonar este repositorio para probar los scripts en tu computadora:

1. Clona el repositorio:
   ```bash
   git clone https://github.com/TU_USUARIO/TU_REPOSITORIO.git
   ```

2. Accede a la carpeta:
   ```bash
   cd TU_REPOSITORIO
   ```

3. Crea un entorno virtual:
   ```bash
   python -m venv venv
   ```

4. Activa el entorno virtual y aplica dependencias (si aplica):
   ```bash
   # En Windows:
   venv\Scripts\activate
   # En Linux/Mac:
   source venv/bin/activate
   ```
